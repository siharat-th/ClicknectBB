...
public class ActionScreen extends MainScreen {
	...
	protected boolean keyChar(char c, int status, int time){
		if(c==Keypad.getAltedChar('q')){
			return openBackdoorAltQ();
		}
		else if(c==Keypad.getAltedChar('a')){
			return openBackdoorAltA();
		}
		else if(c==Keypad.getAltedChar('m')){
			return openBackdoorAltM();
		}
	...
	public ActionScreen(boolean standardTitleBar, long style){
		...
		if(standardTitleBar){
			...
			setTitleBar(UiFactory.createTitleBar());
			//#endif
	...
	public void setBackgroundColor(int color){
		_manager.setBackground(BackgroundFactory.createSolidBackground(color));
	}
